...
export default class Curve extends React.PureComponent {
	...
	computePath() {
		...
		if ( invert ) {
			return `M0,${ height } L0,0 C0,${ ( 1 -
				( leftRatio - rightRatio * oppositeRatio ) ) *
				strength } ${ WIDTH },${ ( 1 - ( rightRatio - leftRatio * oppositeRatio ) ) *
				strength } ${ WIDTH },0 L${ WIDTH },${ height }`;
		}
		return `M0,${ height } C0,${ ( leftRatio - rightRatio * oppositeRatio ) *
			strength } ${ WIDTH },${ ( rightRatio - leftRatio * oppositeRatio ) *
	...
	render() {
		...
		return (
			<svg { ...props } viewBox={ `0 0 ${ WIDTH } ${ ratio * WIDTH }` }>
				<path d={ this.computePath() } style={ { fill: color, stroke: color } } />
			</svg>
		);
	}
}
